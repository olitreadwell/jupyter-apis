...
- 2026-09-25 trivial cleanup — pr-opened https://github.com/olitreadwell/jupyter-apis/pull/6 (branch chore/trivial-cleanup-2026-09-25, base jupyter-apis-aaw2.0 @ 8a69a6a). Whole-repo trivial pass, distinct from PRs #1-#5; every fix verified live in upstream aaw2.0 @8a69a6a and absent from PR #3's diff. Fixed: "occured"->"occurred" in user-facing notebook-status error strings + i18n source catalogs (index-default.component.ts x2, status.go x2, notebooks.go, messages.xlf, messages.fr.xlf); editor README "neccessary"->"necessary", "the editor,you"->"the editor, you", "in from the repo"->"from the repo"; root README "structures"->"structured"; jupyter README "in English. and"->"in English, and" + stale `npm run e2e`/`e2e-ci-all` -> `ui-test`/`ui-test-ci-all`; comment "Enviornment"->"Environment"; dead link nodejs.dev(404)->docs.npmjs.com global-installs. NON-draft, fork CI green (Test, 2x Check code format and lint, 2x Unit tests, UI tests with Cypress all success), mergeable_state clean. English variants respected.
